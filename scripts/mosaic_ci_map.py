...
def main(argv=None):
    ...
    mos.rio.to_raster(rd / "java_n_harvests.tif", compress="lzw")
    a2 = mos.isel(band=0).values

    cmap = ListedColormap(["#ffffff", "#fee08b", "#66bd63", "#1a9850", "#d73027", "#8b0000"])
    norm = BoundaryNorm([-.5, .5, 1.5, 2.5, 3.5, 4.5, 9.5], cmap.N)
    fig, ax = plt.subplots(figsize=(16, 6))
    im = ax.imshow(a2, cmap=cmap, norm=norm,
                   extent=[float(mos.x.min()), float(mos.x.max()), float(mos.y.min()), float(mos.y.max())])
    cb = fig.colorbar(im, ax=ax, ticks=[0, 1, 2, 3, 4, 6], shrink=0.7)
    cb.ax.set_yticklabels(["non-paddy", "1", "2", "3", "4", "5+"]); cb.set_label("harvests / yr")
    ax.set_title(f"Java rice cropping intensity {a.year} (MOGPR S1+S2 phase model, detected paddy)")
    ...
    print(f"wrote {rd}/java_cropping_intensity.png, java_n_harvests.tif, java_production_summary.json")
